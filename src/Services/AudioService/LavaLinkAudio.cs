...
namespace Pyan.Services
{
    public sealed class LavaLinkAudio
    {
        ...
        public async Task<Embed> JoinAsync(IGuild guild, IVoiceState voiceState, ITextChannel textChannel)
        {
            ...
            {
                return await EmbedHandler.CreateErrorEmbed("Music, Join", "I'm already heree!! (╥﹏╥)");
            }

            if (voiceState.VoiceChannel is null)
            {
                return await EmbedHandler.CreateErrorEmbed("Music, Join", "U-umm anon, you must be connected to a voice channel~!");
            }

            try
            {
                await _lavaNode.JoinAsync(voiceState.VoiceChannel, textChannel);
                return await EmbedHandler.CreateBasicEmbed("Music, Join :notes:", $"Joined {voiceState.VoiceChannel.Name} ＼(≧▽≦)／", Color.Green);
            }
            catch (Exception ex)
            {
        ...
        public async Task<Embed> PlayAsync(SocketGuildUser user, IGuild guild, string query)
        {
            ...
            {
                return await EmbedHandler.CreateErrorEmbed("Music, Join/Play", "You Must First Join a Voice Channel (･ω<)~");
            }

            if (!_lavaNode.HasPlayer(guild))
            {
                return await EmbedHandler.CreateErrorEmbed("Music, Play", "I'm not connected to a voice channel you idiot (￣ヘ￣)\nuse the `;join` so I can join the voice channel");
            }
            ...
            try
            {
                ...
                {
                    return await EmbedHandler.CreateErrorEmbed("Music", $"S-sorry! I wasn't able to find anything for {query}.");
                }
                ...
                if (player.Track != null && player.PlayerState is PlayerState.Playing || player.PlayerState is PlayerState.Paused)
                {
                    ...
                    return await EmbedHandler.CreateBasicEmbed3("Music :musical_note:", $"{track.Title} has been added to queue!", Color.Blue);
                }

                await player.PlayAsync(track);
                await LoggingService.LogInformationAsync("Music", $"Bot Now Playing: {track.Title}\nUrl: {track.Url}");
                return await EmbedHandler.CreateBasicEmbed4("Now Playing! :musical_note:", $"{track.Title}\nUrl - {track.Url}", Color.Blue);
            }
        ...
        public async Task<Embed> LeaveAsync(IGuild guild)
        {
            try
            {
                ...
                return await EmbedHandler.CreateBasicEmbed2("Bye, see you~", $"Thank you for using me! ≧◡≦) ♡ .", Color.Gold);
            }
            ...
            try
            {
                ...
                if (player.PlayerState is PlayerState.Playing)
                {
                    ...
                    else
                    {
                        ...
                        return await EmbedHandler.CreateBasicEmbed3("Music Playlist :coffee:", $"Now Playing: \n1. [{player.Track.Title}]({player.Track.Url}) \n{descriptionBuilder}", Color.Blue);
                    }
                }
                else
                {
                    return await EmbedHandler.CreateErrorEmbed("Music, List", "Anon, you better add something into the playlist~.");
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<Embed> SkipTrackAsync(IGuild guild)
        {
            try
            {
                ...
                {
                    return await EmbedHandler.CreateErrorEmbed("Music, SkipTrack", $"A-anon, I can't skip a track as there is only One or No songs currently playing." +
                ...
                else
                {
                    try
                    {
                        ...
                        return await EmbedHandler.CreateBasicEmbed("Music Skip :track_next:", $"I have successfully skiped {currentTrack.Title}", Color.Blue);
                    }
                    catch (Exception ex)
                    {
        ...
        public async Task<Embed> StopAsync(IGuild guild)
        {
            try
            {
                ...
                return await EmbedHandler.CreateBasicEmbed("Music Stop", "I have stopped playback & the playlist has been cleared (*≧ω≦*) ", Color.Blue);
            }
            catch (Exception ex)
            {
        ...
        public async Task<string> PauseAsync(IGuild guild)
        {
            try
            {
                ...
                await player.PauseAsync();
                return $":pause_button: **Paused -** `{player.Track.Title}`";
            }
            catch (InvalidOperationException ex)
            {
                return ex.Message;
        ...
        public async Task<string> ResumeAsync(IGuild guild)
        {
            try
            {
                ...
                return $":notes: **Resumed -** `{player.Track.Title}`";
            }
            catch (InvalidOperationException ex)
            {
                return ex.Message;
        ...
        public async Task TrackEnded(TrackEndedEventArgs args)
        {
            ...
            await args.Player.TextChannel.SendMessageAsync(
                embed: await EmbedHandler.CreateBasicEmbed4("Now Playing", $"[{track.Title}]({track.Url})", Color.Blue));
        }
    }
}
